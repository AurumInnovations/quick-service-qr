...
int mpos_uart_send(char * buf, int count)
{
	...
	if(comm_uart_get_mode() == 0){
		ret = bluetooth_send(buf , count);
		APP_TRACE("send len:%d",ret);
	}
	else{
		ret = comm_uart_send(MPOS_PORT_COM, buf, count);
	}
	return ret;
}
...
int mpos_pub_send_pkt_json(st_pkt_info  * pkt)
{
	char head[7] = {0};
	char end[3] = {0};
	char * buf;
    ...
    if(pkt->func == MPOS_FUNC_RF_POWER)
    {
        if(m_card_type == 1)
            mpay_pub_pack_data(tbuf, "NT", "1");
        else if(m_card_type == 2)
            mpay_pub_pack_data(tbuf, "NT", "2");
        else
            mpay_pub_pack_data(tbuf, "NT", "0");
    }
    else if(pkt->func == MPOS_FUNC_RF_EXCHANGE)
    {
        if(strlen(pkt->pbuff) > 0) 
        {
            mpay_pub_pack_data(tbuf, "D", pkt->pbuff);
        }
    }
